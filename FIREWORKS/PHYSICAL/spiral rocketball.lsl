...
boom()
{
   ...
   //if (!armed)
   //  return;
   debugSay(1,"boom at " + (string)llGetPos() + (string)llGetVel());
...
default
{
   ...
   timer()
   {
      ...
      else
      {
         ...
         {
            debugSay(1,"at low velocity" + (string)v.z);
            llSetTimerEvent(0);
   ...
   collision_start(integer n)
   {
      ...
      key id;
      vector spd;
      debugSay(1,llGetScriptName() + ": collision ");
      if ((explodeOnCollision==0) || (!armed))
         return;
      debugSay(1,llGetScriptName() + ": acting on collision ");
      debugSay(1, "me @ " +(string)llVecMag(llGetVel())+"m/s");
      for (f=0; f<n; f++)
      {
         debugSay(1,llDetectedName(f) + " @ " + (string)llRound(llVecMag(llDetectedVel(f))) + "m/s");
      }
      f = 0;
      do
      {
   ...
   land_collision_start(vector pos)
   {
      if ((explodeOnCollision==0) || (!armed))
         return;
      debugSay(1,"collision with land");
      if (rezParam !=0)
      {
         #if defined TRAILBALL
            AllOff(TRUE);
